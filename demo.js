...
const { getWardById } = require('./dist/wards.js');
...
const { getProvincesBatch, getFullAddressesBatch } = require('./dist/batch.js');
...
async function runDemo() {
	...
	console.log(
		`   Cached result is identical: ${JSON.stringify(sorted1) === JSON.stringify(sorted2)}\n`
	);
	...
	console.time('⚡ Get ward by ID');
	const ward = await getWardById('00004');
	console.timeEnd('⚡ Get ward by ID');
	...
	universalResults.forEach((item) => {
		console.log(
			`   - ${item.type}: ${item.name} ${item.parentName ? `(${item.parentName})` : ''} (score: ${item.score})`
		);
	});
	console.log();
	...
	console.log(`   Hanoi has ${fullHierarchy?.wards.length} wards`);
	...
	const batchAddresses = await getFullAddressesBatch(wardIds);
